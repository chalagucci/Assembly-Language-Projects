...
//  This calculate shall have the following five functionalities:
...
string signed_extension(string s){
    ...
    string addToFront = string(16 - s.length(), s[0]);
    return addToFront + s;
}
...
int binary_to_decimal_signed(string s){
    ...
    if(s[0] == '0')
        ...
        return binary_to_decimal(twos_complement(s)) * (-1);
}

string decimal_to_binary_signed(int n){
    // you implement this one fourth
    string result;
    // if n is non negative then find the decimal_to_binary
    if(n >= 0) result = decimal_to_binary(n);
    else result = decimal_to_binary(-n);
    string add_to_front = string(16 - result.length(), '0');
    result = add_to_front + result;

    if(n < 0) result = twos_complement(result);
    return result;
}
...
string twos_complement(string s){
    ...
    while(rightMost >= 0 && s[rightMost] == '0')
        rightMost--;
